...
const DEFAULT_TO = "info@gyurusimenes.hu";
const DEFAULT_FROM = "Gyűrűsi Ménes <weboldal@gyurusimenes.hu>";
...
export function mailStatus() {
  return { enabled: !!process.env.RESEND_API_KEY, to: contactTo(), from: process.env.CONTACT_FROM?.trim() || DEFAULT_FROM, customBase: apiBase() !== DEFAULT_BASE };
}
...
async function send(payload: { to: string[]; subject: string; text: string; reply_to?: string }): Promise<Result> {
  ...
  try {
    const res = await fetch(`${apiBase()}/emails`, {
      ...
      body: JSON.stringify({ from, ...payload }), cache: "no-store", signal: AbortSignal.timeout(TIMEOUT_MS),
    });
    if (!res.ok) return { sent: false, reason: `Resend ${res.status}: ${(await res.text().catch(() => "")).slice(0, 300)}` };
    return { sent: true };
  } catch (e) {
    return { sent: false, reason: `Resend nem elérhető: ${e instanceof Error ? e.message : String(e)}` };
  }
}
...
export async function sendContactMail(input: { name: string; email: string; phone?: string; message: string; page?: string }): Promise<Result> {
  ...
  return send({ to: [contactTo()], reply_to: input.email, subject: `Új üzenet a weboldalról${input.page ? ` (${input.page})` : ""} – ${input.name}`, text });
}
...
export async function sendRegistrationMail(input: { eventTitle: string; when: string; name: string; phone: string; email?: string; count: number; note?: string }): Promise<Result> {
  ...
  return send({ to: [contactTo()], reply_to: input.email, subject: `Jelentkezés: ${input.eventTitle} – ${input.name} (${input.count} fő)`, text });
}
...
export async function sendRegistrationConfirmation(input: { to: string; lang: "hu" | "en" | "de"; eventTitle: string; when: string; phone: string }): Promise<Result> {
  ...
  return send({ to: [input.to], reply_to: contactTo(), ...m });
}
