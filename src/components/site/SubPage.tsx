...
export function Shell({ site, lang, d, rest, phone, dockHref, children }: { site: SiteContent; lang: Lang; d: Dictionary; rest: string; phone?: string; dockHref?: string; children: React.ReactNode }) {
  const p = phone ?? site.contact.phone;
  return (
    <>
      ...
      <ContactDock phone={p} labels={{ message: d.contact.sub.write, call: d.contact.sub.call }} href={dockHref ?? `mailto:${site.contact.email}`} />
    </>
  );
}

/**
 * Aloldal-keret: cím a képfej fölött; kétoszlopos törzs — balra a szöveg (és a képsáv),
 * jobbra ragadós oszlop a tényekkel és AZ ADOTT OLDAL SAJÁT kapcsolatával (személy, telefon,
 * e-mail, megjegyzés); alatta teljes szélességű „after” blokk (jelentkezés / beszámolók),
 * végül kapcsolódó kártyák. Ugyanaz a ritmus minden aloldalon.
 */
export function SubPage({ site, lang, d, rest, eyebrow, title, meta, image, strip = [], contact, facts = [], related = [], relatedTitle, back, after, children }: {
  ...
  return (
    <Shell site={site} lang={lang} d={d} rest={rest} phone={c.phone} dockHref={`mailto:${c.email}`}>
      ...
      <div className="wrap sub-grid">
        <div className="sub-body">
          ...
          {strip.length > 0 && (
            <Reveal className="sub-strip" delay={60}>
              {strip.map((im, i) => <figure key={i} className="photo"><Image src={im.src} alt={im.alt} width={im.width} height={im.height} sizes="(max-width: 640px) 50vw, 25vw" quality={62} placeholder={im.blur ? "blur" : "empty"} blurDataURL={im.blur} style={{ backgroundColor: im.color }} /></figure>)}
            </Reveal>
          )}
        </div>
        <aside className="sub-aside">
          ...
          <Reveal className="sub-contact" delay={80} data-page-contact>
            ...
            <a href={`mailto:${c.email}`} className="btn btn-primary">{d.contact.sub.write} <Arrow /></a>
          </Reveal>
        </aside>
      </div>
...
export const home = (lang: Lang) => langPath(lang);
