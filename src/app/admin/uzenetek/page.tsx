...
export default async function MessagesAdmin() {
  ...
  return (
    <>
      <div className="adm-head"><div><h1>Üzenetek</h1><p>A kapcsolati űrlapon beérkezett megkeresések. E-mailben is megkapod, ha be van állítva.</p></div></div>
        ...
        <div className="list">{msgs.map((m) => (
          <article key={m.id} className={`msg ${m.read ? "" : "unread"}`}>
            <header>
              ...
              <div className="note">{new Intl.DateTimeFormat("hu-HU", { dateStyle: "medium", timeStyle: "short" }).format(new Date(m.receivedAt))}</div>
            </header>
            <pre>{m.message}</pre>
